...
def minimax(board, depth, player):
    ...
    length = len(Moves)
    if CheckWin(board):
        return depth+simplescore(board, switchplayers(player)), None
    ...
    if player == 'X':
        ...
        for move in Moves:
            ...
            Score = minimax(theBoard, depth-1, switchplayers(player))[0]
            theBoard[move] = ' '
            if Score > Max:
                Max = Score
                Move = move
                if Max >= 20: #Pass a win straight up. Stop Searching.
                    break
        return Max, Move
    else:
        Min = 100
        for move in Moves:
            ...
            if Score < Min:
                Min = Score
                Move = move
                if Min <= -10: #Pass a loss straight up. Stop Searching.
                    break
        return Min, Move
###############################################################################  
...
def game():
    ...
    for i in range(20):
        ...
        if turn == 'O':
            ...
            Start = datetime.now()
            score, bestmove = minimax(theBoard,9,turn)
            Stop = datetime.now()
            print(score)
            print(Stop-Start)
